...
/**
 * 愛外送 報表統計
 * php artisan Reports:ReportBillingGenJob --company=3 --start_datetime=20180808010000
 */
class ReportBillingGenJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Reports:ReportBillingGenJob {--company= : 品牌ID} {--start_datetime= : 查詢日EX=20180101010000}';
    ...
    public function handle()
    {
        ...
        foreach($company_arr as $company_id) {
            ...
            $billing_src = Billing::where("company_id", '=', $company_id)
                ->whereNotNull("created_time")//開帳日期不為空
                //->where("status", 1)//開立、作廢之結帳單
                ->whereBetween("created_time", [$start_at, $end_at])//符合起迄時段
                ->where("service_id", env("SERVICE_ID"))
                ->select("id", "company_id", "store_id", "status",
                    DB::raw("concat(date(created_time), ' ', hour(created_time)) as report_at"),
                    "product_delivery", "member_id", "amount_src", "total_qty",
                    "amount_campaign", "amount_coupon", "amount_custom",
                    "amount");

            $billing = $billing_src->select("company_id", "store_id",
                DB::raw("concat(date(created_time), ' ', hour(created_time)) as report_at"),
                "product_delivery", "member_id", DB::raw("
                count(case when status = 1 then id end) as order_count,
                SUM(case when status = 1 then amount_src end) as amount_src, 
                SUM(case when status = 1 then total_qty end) as total_qty, 
                SUM(case when status = 1 then amount_campaign end) as amount_campaign, 
                SUM(case when status = 1 then amount_coupon end) as amount_coupon, 
                SUM(case when status = 1 then amount_custom end) as amount_custom, 
                SUM(case when status = 1 then amount end) as amount,
                SUM(case when status = 0 then amount end) as cancel_amount,
                count(case when status = 0 then id end) as cancel_count"))
                ->groupBy("company_id", "store_id", "report_at", "product_delivery",
                    "member_id")
            ...
            if (!$billing->isEmpty()) {
                ...
                foreach ($billing as $order) {
                    ...
                    $order_input["src_amount"] = empty($order->amount_src) ? 0 : $order->amount_src;
                    $order_input["total_qty"] = empty($order->total_qty) ? 0 : $order->total_qty;
                    $order_input["discount_amount"] = empty($order->amount_campaign) ? 0 : $order->amount_campaign;
                    $order_input["coupon_discount"] = empty($order->amount_coupon) ? 0 : $order->amount_coupon;
                    $order_input["custom_discount"] = empty($order->amount_custom) ? 0 : $order->amount_custom;
                    $order_input["amount"] = empty($order->amount) ? 0 : $order->amount;
                    $order_input["cancel_amount"] = empty($order->cancel_amount) ? 0 : $order->cancel_amount;
                    $order_input["cancel_count"] = empty($order->cancel_count) ? 0 : $order->cancel_count;
    ...
    private function add_report_order($order_input)
    {
        $db = Report_order::updateOrCreate(["company_id" => $order_input["company_id"],
            ...
            "member_class" => $order_input["member_class"],
            ...
            ["order_count" => $order_input["order_count"],
                ...
                "cancel_amount" => $order_input["cancel_amount"], "cancel_count" => $order_input["cancel_count"]]);
