...
class Orders extends Model
{
    ...
    protected $fillable = [
        ...
        'store_name', 'member_id', 'member_name', 'payment', 'payment_status',
        'pay_callback', 'payment_time', 'product_delivery', 'status', 'amount',
        'total_qty', 'delivery_post_code', 'delviery_address', 'prefer_datetime', 'create_time'
    ];
    // 隱藏不顯示欄位
    //protected $hidden = [];
    // 軟刪除
    ...
    {
        return $this->hasMany(OrdersDtail::class);
    }

    public static function getAddress($id)
    {
        $rows = DB::table('order')->where('id', '=', $id)->get();
        ...
        if (!empty($rows[0]->delivery_post_code))
        {
            $result = self::getZip()[$rows[0]->delivery_post_code]['city']. self::getZip()[$rows[0]->delivery_post_code]['area'] .'<br/>'. $rows[0]->delivery_address;
        }

        return $result;
    }

    public static function getGridView()
    {
        return DB::select('select id, sn, member_name, purchase_phone, product_delivery, prefer_datetime, payment, 
                    total_qty, amount, status, (select concat(delivery_post_code,delivery_address) from `order` b 
                    where a.id=b.id) as delivery_address from `order` a');
    }
     ...
     * @var company_id 公司編號
     */
    public static function getThisMonthIncome($company_id)
    {
        $datetime = new \DateTime('now');

        return DB::table('order')
                    ->where(array(['status', '6'],['company_id', $company_id]))
                    ->whereBetween('create_time', [$datetime->format('Y-m-d'), $datetime->format('Y-m-d')])
                    ->sum('amount');
    }
     ...
     * @var company_id 公司編號
     */
    public static function getLastMonthIncome($company_id)
    {
        $datetime = new \DateTime('now');
     ...
     * @var store_id 分店編號
     */
    public static function getStoreTodayOrders($store_id)
    {
        $datetime = new \DateTime('now');
        
        return DB::table('order')
                    ->where(array(['status', '6'],['store_id', $store_id],['create_time', $datetime->format('Y-m-d')]))
                    ->count();
    }
     ...
     * @var store_id 分店編號
     */
    public static function getStoreTodayOrdersAmount($store_id)
    {
        $datetime = new \DateTime('now');
     ...
     * @var store_id 分店編號
     */
    public static function getStoreMonthlyOrders($store_id)
    {
        $datetime = new \DateTime('now');

        return DB::table('order')
                    ->where(array(['store_id', $store_id],['create_time', $datetime->format('Y-m')]))
                    ->count();
    }
     ...
     * @var store_id 分店編號
     */
    public static function getStoreMonthlyOrdersAmount($store_id)
    {
        $datetime = new \DateTime('now');
     ...
     * @var store_id 分店編號
     */
    // public static function getProductExchange($service_id, $company_id, $store_id)
    // {
    //     $result = self::join('product_exchange', 'product_exchange.orders_id', '=', 'order.id')
    //         ->join('exchanges', 'exchanges.id', '=', 'product_exchange.exchanges_id')
    ...
    //         ->select('product_exchange.date', 'exchanges.name', 'exchanges.image', 'exchanges.point_type_id', 
    //             'exchanges.point', 'product_exchange.qty', DB::raw('exchanges.point * product_exchange.qty as total_point'), 
    //             'product_exchange.orders_id', 'order.sn', 'product_exchange.status', 'product_exchange.id')
    //         ->orderBy('order.sn', 'desc'); 
