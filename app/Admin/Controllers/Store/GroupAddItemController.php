...
class GroupAddItemController extends Controller
{
    ...
    public function edit($id)
    {
        return Admin::content(function (Content $content) use ($id) {
            ...
            if (empty($menu_item) || empty($menu_item->store_id)
    ...
    protected function grid()
    {
        Admin::script('$(".price_submit").on("click", function () {
            ...
            $.ajax({
                url: "/admin/store/set/menu_item/ajax",
                type: "POST",
                dataType : "json",
    ...
    ');
        ...
        return Admin::grid(MenuItem::class, function (Grid $grid) {
            ...
            $grid->model()->where('company_id', Session::get('company_id'))
                    ->where('store_id', Session::get('store_id'))->orderby('sort_by', 'asc');
    ...
    protected function form()
    {
        return Admin::form(MenuItem::class, function (Form $form) {
            ...
            $form->hidden('store_id','品牌編號')->default(Session::get('store_id'));
            ...
            if (!empty($unit)) {
                ...
                foreach ($unit as $unit_row) {
                    ...
                    // if ($unit_row['is_multiple'] == '1') //checkbox
                    // {
                    //     $form->checkbox('unit_name_'.$i, $unit_row['unit_name'])->options($attr_opt)->default($default);
                    // }
                    // else //radio
                    // {
                    //     $form->radio('unit_name_'.$i, $unit_row['unit_name'])->options($attr_opt)->default($default);
                    // }

                    // $form->ignore('unit_name_'.$i);
            ...
            $form->saved(function (Form $form) use ($options_list) {
                ...
                MenuItem::addMenuStoreItem(Session::get('store_id'), $form->model()->id);
