...
class CouponItemAmountController extends Controller
{
    use ModelForm;
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header(trans('coupon.item.amount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->condition_qty_menu_item()->value(trans('coupon.item.cond_value'));
            $grid->offer_amount_menu_item()->value(trans('coupon.item.amount.offer_value'));
        ...
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                $form->radio("product_delivery", trans('coupon.field.product_delivery'))
                    ...
                    ->default(0);
                $form->checkbox('condition_qty_menu_item.menu_item_ids', trans('coupon.item.cond_menu_item'))
                    ->options(Menu_item::where('company_id', $company_id)->where('store_id', $store_id)->pluck('name', 'id'))
                    ->rules('required');
                $form->text("condition_qty_menu_item.value", trans('coupon.amount.cond_value'))
                    ->default(0)
                    ->rules('required|min:0')
                    ->attribute('id', 'cond_value')
                    ->attribute('style', 'width:150px;');
            })->tab(trans('coupon.offer_tab'), function ($form) {
                $form->text("offer_amount_menu_item.value", trans('coupon.item.amount.offer_value'))
                    ->default(0)
                    ->rules('required|min:0')
                    ->attribute('id', 'offer_value')
                    ->attribute('style', 'width:150px;');

                $form->text('offer_amount_menu_item.max_value', trans('coupon.item.amount.offer_max_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('coupon.item.amount.offer_max_value').'必須是數字']);
