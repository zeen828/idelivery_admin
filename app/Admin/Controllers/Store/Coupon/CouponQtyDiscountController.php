...
class CouponQtyDiscountController extends Controller
{
    use ModelForm;
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header(trans('coupon.qty.discount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->condition_qty()->value(trans('coupon.qty.cond_value'));
            $grid->offer_discount()->value(trans('coupon.qty.discount.offer_value'));
        ...
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                $form->radio("product_delivery", trans('coupon.field.product_delivery'))
                    ...
                    ->default(0);
                $form->text("condition_qty.value", trans('coupon.qty.cond_value'))
                    ->default(0)
                    ->rules('required|min:0')
                    ->attribute('id', 'cond_value')
                    ->attribute('style', 'width:150px;');
            })->tab(trans('coupon.offer_tab'), function ($form) {
                $form->rate("offer_discount.value", trans('coupon.qty.discount.offer_value'))
                    ->rules("required | between:1,99.99")
                    ->attribute('id', 'offer_value');
                // $form->text("offer_amount.max_value", trans('coupon.field.offer_max_value'))
                //     ->default(0)
                //     ->rules('required|min:0')
