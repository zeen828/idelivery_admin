...
class CouponItemDiscountController extends Controller
{
    use ModelForm;
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header(trans('coupon.item.discount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()->where('types', 2)
                ...
                ->where('offer_table', '=', 'Offer_discount_menu_group');

            $grid->id(trans('coupon.field.id'))->sortable();
            $grid->title(trans('coupon.field.title'));
            $grid->description(trans('coupon.field.description'));
            $grid->condition_qty_menu_group()->value(trans('coupon.item.cond_value'));
            $grid->offer_discount_menu_group()->value(trans('coupon.item.discount.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_discount_menu_group');
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                $form->checkbox('condition_qty_menu_group.menu_item_ids', trans('coupon.item.cond_menu_item'))
                    ->options(Menu_item::where('company_id', $company_id)->where('store_id', $store_id)->pluck('name', 'id'))
                    ->rules('required');

                $form->text("condition_qty_menu_group.value", trans('coupon.item.cond_value'))
                    ->default(0)
                    ->rules('required|min:0')
                    ->attribute('id', 'cond_value')
                    ->attribute('style', 'width:150px;');
            })->tab(trans('coupon.offer_tab'), function ($form) {
                $form->rate("offer_discount_menu_group.value", trans('coupon.item.discount.offer_value'))
                    ->rules("required | between:1,99.99")
                    ->attribute('id', 'offer_value');

                $form->text('offer_discount_menu_group.max_value', trans('coupon.item.discount.offer_max_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('coupon.item.discount.offer_max_value').'必須是數字']);
