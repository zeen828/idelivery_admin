...
class PeriodSalesController extends Controller
{
    public function index()
    {
        return Admin::content(function (Content $content) {

            $content->header('時段銷售統計表');
            $content->description('Daily Sales Report');

            $route = "/admin/store/reports/period_sale/search";
    ...
    public function search()
    {
        return Admin::content(function (Content $content) {
            ...
            if (!empty(request()->all()))
            {
                ...
                $content->row(new Box('統計表', view('admin.reports.period_sale_chart'), ['start_at'=>$start_at, 'end_at'=>$end_at, 'rows'=>$rows]));

                $sum = $this->sum_data($store_id, $start_at, $end_at);

                $box = new Box($msg, view('admin.reports.period_sale_table', ['rows'=>$rows, 'sum'=>$sum]));
    ...
    {
        //取得區間內銷售金額不為 0之最早及最晚時段
        $period = $this->get_period($store_id, $start_at, $end_at);
        $rows = null;

        if (!empty($period))
        {
            $rows = Report_order::where('start_hour', '>=', $period->start_at)
                ->where('end_hour', '<=', $period->end_at)
                ->whereBetween('hours', [ (int) date("H", strtotime($period->first_at)), (int) date("H", strtotime($period->last_at))])
                ->where('store_id', $store_id)
                ->select("hours", DB::raw("sum(order_count) as cnt, sum(src_amount) as src_amount,
                    ...
                    sum(amount) as amount, sum(cancel_amount) as cancel_amount,
    ...
    //取得區間內銷售金額不為 0之最早及最晚時段
    private function get_period($store_id, $start_at, $end_at)
    {
        $report_order = Report_order::where('start_hour', '>=', $start_at)
            ->where('end_hour', '<=', $end_at)
            ->where('store_id', $store_id)
            ->where(function($query)
            {
                $query->where('amount', '<>', 0)
                    ->orWhere('cancel_amount', '<>', 0);
            })
            ->orderBy('start_hour')
            ->get();

        if (count($report_order) == 0)
        {
            return null;
        }

        $first_at = $report_order->first()->start_hour;
        $last_at = $report_order->last()->start_hour;

        $start_at = date("Y-m-d", strtotime($start_at))." ".date("H:00:00", strtotime($first_at));
        $end_at = date("Y-m-d", strtotime($end_at))." ".date("H:59:59", strtotime($last_at));

        $period = new \StdClass();
        $period->start_at = $start_at;
        $period->end_at = $end_at;
        $period->first_at = $first_at;
        $period->last_at = $last_at;
