...
class ItemController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.item.qty.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_menu_item');
            ...
            $grid->offer_menu_item()->value(trans('campaign.item.qty.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                $form->text('offer_menu_item.value', trans('campaign.item.qty.offer_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.item.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');

                $form->text('offer_menu_item.max_value', trans('campaign.item.qty.offer_max_value'))
                    ->default(0)
