...
class AmountForQtyController extends Controller
{
    use ModelForm;
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.amount.qty.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('condition_table', 'Condition_amount')
            ...
            $grid->condition_amount()->value(trans('campaign.amount.cond_value'));
            $grid->offer_qty()->value(trans('campaign.amount.qty.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            $form->hidden('company_id')->default(Session::get('company_id'));
            $form->hidden('store_id')->default(Session::get('store_id'));
            $form->hidden('event_id')->default(Session::get('event_id'));
            $form->hidden('condition_table')->default('Condition_amount');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                $form->text('condition_amount.value', trans('campaign.amount.cond_value'))
                ...
                $form->text('offer_qty.value', trans('campaign.amount.qty.offer_value'))
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.amount.qty.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');
                
                $form->text('offer_qty.max_value', trans('campaign.amount.qty.offer_max_value'))
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.amount.qty.offer_max_value').'必須是數字']);
