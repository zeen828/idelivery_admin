...
class ItemForAmountController extends Controller
{
    use ModelForm;
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.item.amount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_amount_menu_item');
            ...
            $grid->condition_qty_menu_item()->value(trans('campaign.item.cond_value'));
            $grid->offer_amount_menu_item()->value(trans('campaign.item.amount.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_amount_menu_item');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                $form->checkbox('condition_qty_menu_item.menu_item_ids', trans('campaign.item.cond_menu_item'))
                    ->options(Menu_item::where(['company_id' => $company_id, 'store_id' => $store_id])->pluck('name', 'id'))
                    ->rules('required');

                $form->text('condition_qty_menu_item.value', trans('campaign.item.cond_value'))
                    ->default(0)
                ...
                $form->text('offer_amount_menu_item.value', trans('campaign.item.amount.offer_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.item.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');

                $form->text('offer_amount_menu_item.max_value', trans('campaign.item.amount.offer_max_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.amount.offer_max_value').'必須是數字']);
