...
class DiscountForItemController extends Controller
{
    use ModelForm;
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.item.discount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_menu_item_discount');
            ...
            $grid->offer_menu_item_discount()->value(trans('campaign.item.discount.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_menu_item_discount');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                $form->rate("offer_menu_item_discount.value", trans('campaign.item.discount.offer_value'))
                    ->default(0)
                    ->rules('required | between:1,99.99',['regex' => trans('campaign.item.discount.offer_value').'必須是數字'])    
                    ->attribute('id', 'offer_value');

                $form->text('offer_menu_item_discount.max_value', trans('campaign.item.discount.offer_max_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.discount.offer_max_value').'必須是數字']);
