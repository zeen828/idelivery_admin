...
class ItemForRandomController extends Controller
{
    use ModelForm;
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.item.item.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->condition_menu_item()->value(trans('campaign.item.cond_value'));
            $grid->offer_menu_item_random()->value(trans('campaign.item.item.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_menu_item_random');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                $form->checkbox('condition_menu_item.menu_item_ids', trans('campaign.item.cond_menu_item'))
                    ->options(Menu_item::where('company_id', $company_id)->whereIn('store_id', [0, $store_id])->pluck('name', 'id'))
                    ->rules('required');

                $form->text('condition_menu_item.value', trans('campaign.item.cond_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.cond_value').'必須是數字'])
                    ->attribute('id', 'cond_value');
                // $form->multipleSelect('condition_menu_item.value', '選擇品項')
                //     ->options(Menu_item::where('company_id', $company_id)->pluck('name', 'id'))
                //     ->rules('required');
            })->tab(trans('campaign.offer_tab'), function($form) use ($company_id, $store_id) {
                $form->checkbox('offer_menu_item_random.menu_item_ids', trans('campaign.item.item.offer_menu_item'))
                    ->options(Menu_item::where('company_id', $company_id)->whereIn('store_id', [0, $store_id])->pluck('name', 'id'))
                    ->rules('required');

                $form->text('offer_menu_item_random.value', trans('campaign.item.item.offer_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.item.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');

                $form->text('offer_menu_item_random.max_value', trans('campaign.item.item.offer_max_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.item.item.offer_max_value').'必須是數字']);
