...
class QtyForNAmountController extends Controller
{
    use ModelForm;
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) {
            $content->header(trans('campaign.qty.amount_item_group_n.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_amount_menu_group_n');
            ...
            $grid->offer_amount_menu_group_n()->value(trans('campaign.qty.amount_item_group_n.offer_value'));
            $grid->offer_amount_menu_group_n()->n_th(trans('campaign.qty.amount_item_group_n.nth'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_amount_menu_group_n');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                // 第幾件有優惠
                $form->text('offer_amount_menu_group_n.n_th', trans('campaign.qty.amount_item_group_n.nth'))
                    ->default(0)
                    ->help('商品價錢由低到高排序')
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.amount_item_group_n.nth').'必須是數字'])
                    ->attribute('id', 'offer_nth');
                // 優惠多少
                $form->text('offer_amount_menu_group_n.value', trans('campaign.qty.amount_item_group_n.offer_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.amount_item_group_n.offer_value').'必須是數字'])
            ...
            $form->saved(function (Form $form) {
                ...
                $campaign_setting = Campaign_setting_form::find($id);
                $campaign_setting->offer_amount_menu_group_n->menu_item_ids = $campaign_setting->condition_qty_menu_group->menu_item_ids;
                $campaign_setting->offer_amount_menu_group_n->save();
