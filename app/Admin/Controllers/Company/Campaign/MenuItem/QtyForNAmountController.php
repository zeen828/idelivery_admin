...
class QtyForNAmountController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_amount_menu_item_n');
            ...
            $grid->offer_amount_menu_item_n()->n_th('第幾件');
            $grid->offer_amount_menu_item_n()->value(trans('campaign.qty.amount_item_group_n.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_amount_menu_item_n');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                // 第幾件有優惠
                $form->text('offer_amount_menu_item_n.n_th', '第幾件')
                    ->default(1)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => '[第幾件]必須是數字'])
                    ->attribute('id', 'offer_value');
                // 優惠多少錢
                $form->text('offer_amount_menu_item_n.value', trans('campaign.qty.amount_item_group_n.offer_value'))->default(0);
            ...
            $form->saved(function (Form $form) {
                ...
                $campaign_setting = Campaign_setting_form::find($id);
                $campaign_setting->offer_amount_menu_item_n->menu_item_ids = $campaign_setting->condition_qty_menu_item->menu_item_ids;
                $campaign_setting->offer_amount_menu_item_n->save();
