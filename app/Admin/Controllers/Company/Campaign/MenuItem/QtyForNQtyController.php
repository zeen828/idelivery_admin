...
class QtyForNQtyController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()
                ...
                ->where('offer_table', 'Offer_qty_menu_item_n');
            ...
            $grid->offer_qty_menu_item_n()->n_th('第幾件');
            $grid->offer_qty_menu_item_n()->price(trans('campaign.qty.qty_item_group_n.price'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('offer_table')->default('Offer_qty_menu_item_n');
            ...
            $form->tab(trans('campaign.setting_tab'), function($form) {
                ...
                // 第幾件優惠
                $form->text('offer_qty_menu_item_n.n_th', '第幾件')
                    ->default(1)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => '[第幾件]必須是數字'])
                    ->attribute('id', 'offer_value');
                // 預設一件
                $form->hidden('offer_qty_menu_item_n.value')->default(1);
                // 優惠多少錢
                $form->text('offer_qty_menu_item_n.price', trans('campaign.qty.qty_item_group_n.price'))
                    ->default(0)
                ...
                // 最多幾件優惠
                $form->text('offer_qty_menu_item_n.max_value', trans('campaign.qty.qty.offer_max_value'))
                    ->default(0)
            ...
            $form->saved(function (Form $form) {
                ...
                $campaign_setting = Campaign_setting_form::find($id);
                $campaign_setting->offer_qty_menu_item_n->menu_item_ids = $campaign_setting->condition_qty_menu_item->menu_item_ids;
                $campaign_setting->offer_qty_menu_item_n->save();
