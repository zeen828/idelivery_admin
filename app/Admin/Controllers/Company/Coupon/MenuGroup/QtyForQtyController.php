...
class QtyForQtyController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->offer_qty_menu_group_n()->value(trans('campaign.qty.qty_item_group.offer_value'));
            $grid->offer_qty_menu_group_n()->price(trans('campaign.qty.qty_item_group.price'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                // 預設一件
                $form->text('offer_qty_menu_group.value')
                    ->default(1)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.qty_item_group_n.price').'必須是數字'])
                    ->attribute('id', 'offer_value');
                // 優惠多少錢
                $form->text('offer_qty_menu_group.price', trans('campaign.qty.qty_item_group.price'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.qty_item_group_n.price').'必須是數字'])
                    ->attribute('id', 'offer_value');
