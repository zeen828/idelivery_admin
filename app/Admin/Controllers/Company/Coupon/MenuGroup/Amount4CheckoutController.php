...
class Amount4CheckoutController extends Controller
{
    use ModelForm;
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header(trans('campaign.amount.amount.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->model()->where('types', 2)
                ...
                ->where('condition_table', '=', 'Condition_amount_menu_group')
            ...
            $grid->column(trans('campaign.amount.menu_item'))->display(function() {
                $menu_item = Menu_item::whereIn('id', $this->condition_amount_menu_group->menu_item_ids)->get();
            ...
            $grid->offer_amount()->value(trans('campaign.amount.amount.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->hidden('condition_table')->default('Condition_amount_menu_group');
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                $form->text("user_use_count", trans('coupon.field.user_use_count'))
                    ...
                    ->rules('required|min:0')
                    ->attribute('style', 'width:150px;');
                $form->textarea("remark", trans('coupon.field.remark'))->rows(5)
                    ->default($remark_txt);
                ...
                $form->checkbox('condition_amount_menu_group.menu_item_ids', trans('campaign.amount.menu_item'))
                    ->options(Menu_item::where(['company_id' => $company_id, 'store_id' => $store_id])->pluck('name', 'id'))
                    ->rules('required');

                $form->text('condition_amount_menu_group.value', trans('campaign.amount.cond_value'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.amount.cond_value').'必須是數字'])
                    ->attribute('id', 'cond_value');
            })->tab(trans('coupon.offer_tab'), function ($form) {
                $form->text('offer_amount.value', trans('campaign.amount.amount.offer_value'))
                    ->default(0)
                ...
                $form->divide();
                $form->switch("repeat", trans('campaign.field.repeat'))->states($this->repeat_arr);
            });
