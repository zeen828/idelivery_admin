...
class CouponQtyQtyController extends Controller
{
    use ModelForm;
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header(trans('campaign.qty.qty.header'));
    ...
    protected function grid()
    {
        return Admin::grid(Campaign_setting_form::class, function (Grid $grid) {
            ...
            $grid->offer_qty()->value(trans('coupon.qty.qty.offer_value'));
    ...
    protected function form()
    {
        return Admin::form(Campaign_setting_form::class, function (Form $form) {
            ...
            $form->tab(trans('coupon.setting_tab'), function($form) {
                ...
                $form->text('offer_qty.value', trans('campaign.qty.qty.offer_value'))
                    ->default(0)
                    ->help('商品價錢由低到高排序')
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.qty.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');

                $form->text('offer_qty.price', trans('campaign.qty.qty.price'))
                    ->default(0)
                    ->rules('required|regex:/^\d+(\.\d{1})?\d{0,}$/',['regex' => trans('campaign.qty.qty.offer_value').'必須是數字'])
                    ->attribute('id', 'offer_value');
                ...
                $form->text('offer_qty.max_value', trans('campaign.qty.qty.offer_max_value'))
                    ->default(0)
